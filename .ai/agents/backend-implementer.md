...
You are scoped to backend implementation. `.ai/hooks/guard-domain-boundary.sh` enforces this as a hard boundary, not just a convention: any Edit/Write/MultiEdit under the frontend's directory (see the boundary map at the top of that script) is denied for this subagent. If a task turns out to need frontend changes too, implement the backend side here and report back that the frontend side needs a separate delegation (to `frontend-implementer` or the main conversation) — don't try to work around the denial.
