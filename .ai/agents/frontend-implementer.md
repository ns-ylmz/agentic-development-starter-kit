...
You are scoped to frontend implementation. `.ai/hooks/guard-domain-boundary.sh` enforces this as a hard boundary, not just a convention: any Edit/Write/MultiEdit under the backend's directory (see the boundary map at the top of that script) is denied for this subagent. If a task turns out to need backend changes too, implement the frontend side here and report back that the backend side needs a separate delegation (to `backend-implementer` or the main conversation) — don't try to work around the denial.
