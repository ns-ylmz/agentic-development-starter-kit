...
This directory owns task plans and their lifecycle. `.ai/task-workflow.md → Task Decomposition Standards` defines what a bounded task is; this directory is where those tasks live as files, so execution is repeatable and reviewable.
...
1. Copy `templates/task-plan.md` into `active/` and fill every section. A plan is ready for execution when its scope can be verified before implementation starts (see `.ai/task-workflow.md → Task Decomposition Standards`).
2. Execute it with the `execute-task` skill (or the Task-File Execution Prompts in `.ai/prompt-templates.md`). The task file is authoritative for scope, constraints, verification, and completion criteria.
...
- Plans reference documentation by path (e.g. `.ai/architecture-rules.md → Runtime Contract Rules`) rather than restating rules.
