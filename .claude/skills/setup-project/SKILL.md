...
3. **Boundary map**: update the map at the top of `.claude/hooks/guard-domain-boundary.sh` to the real layout and the agents that remain.
...
8. **Tooling wiring**: ensure root `package.json` has the `prepare: husky` script, the lint-staged block from README.md, and the dev dependencies (husky, lint-staged, commitlint, prettier, eslint). Run `chmod +x .claude/hooks/*.sh .husky/pre-commit .husky/commit-msg`.
