...
- **Hooks** (`.claude/settings.json` → `.ai/hooks/*.sh`): `PreToolUse` on `Bash` runs `guard-git.sh` — blocks commits on `main`/`master`, force-push, and staged secret/env files. `PreToolUse` on `Bash` also runs `guard-pr-body.sh` — blocks PR creation unless the body follows `.github/pull_request_template.md`. `PostToolUse` on `Bash` runs `suggest-branch-cleanup.sh`. `PostToolUse` on `Bash` and `mcp__.*__pull_request_create` runs `remind-return-to-main.sh`.
- **Skills** (`.claude/skills/*/SKILL.md`): `implement`, `refactor`, `add-tests`, `update-docs`, `bugfix`, `execute-task` — each wraps the matching section of `.ai/prompt-templates.md`. 
